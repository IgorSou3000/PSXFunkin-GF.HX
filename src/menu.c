...
void Menu_Load(MenuPage page)
{
	...
	for (u8 i = 0; i < COUNT_OF(Sounds_Text); i++)
	{
		...
		CdlFILE sfx_file;
		IO_FindFile(&sfx_file, sfx_text);
		u32 *data = IO_ReadFile(&sfx_file);
		menu.sounds[i] = Audio_LoadSFXData(data, sfx_file.size);
		Mem_Free(data);
	}
...
void Menu_Tick(void)
{
	...
	//Fallthrough
		case MenuPage_Title:
		{
			...
			{
				//Play Confirm Sound
				Audio_PlaySFX(menu.sounds[1], 0x2fff);
		...
		case MenuPage_Main:
		{
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				{
					//Play Scroll Sound
					 Audio_PlaySFX(menu.sounds[0], 0x2fff);
					if (menu.select > 0)
						menu.select--;
					else
				...
				{
					//Play Cancel Sound
					 Audio_PlaySFX(menu.sounds[2], 0x2fff);
		...
		case MenuPage_Story:
		{
			static const struct
			{
				...
				{NULL, StageId_1_1, "HX", {"INVICIBLE", NULL, NULL}},
			};
		...
		case MenuPage_Freeplay:
		{
			static const struct
			{
				...
				{StageId_1_1, 0xFF90124C, "INVICIBLE"},
			};
		...
		case MenuPage_Credits:
		{
			static const struct
			{
				...
				{"IGORSOU", 0XFFFFD400},
				{"", NULL},
				{"OG CREATORS", 0XFF00FF23},
				{"", NULL},
				{"VANILLANOMAS WASNT HERE", 0XFF00FF23},
				{"BIDDLE", 0XFF00FF23},
				{"RANDOMINC", 0XFF00FF23},
				{"VIVADERUS", 0XFF00FF23},
				{"ITOSAIHARA", 0XFF00FF23},
				{"MURASAKI", 0XFF00FF23},
				{"", NULL},
				{"ENGINE BY", 0XFFFD5900},
				{"", NULL},
				{"IGORSOU", 0XFFFD5900},
				{"SPICYJPEG", 0XFFFD5900},
				{"", NULL},
				{"SPECIAL THANKS", 0XFF0023FD},
				{"", NULL},
				{"CUCKYDEV" , 0XFF0023FD},
				{"BILIOUS", 0XFF0023FD},
				{"LORD SCOUT", 0XFF0023FD},
				{"UNSTOPABLE", 0XFF0023FD},
				{"SPARK", 0XFF0023FD},
				{"ZERIBEN", 0XFF0023FD},
				{"LUCKY", 0XFF0023FD},
				{"BIG FLOPPA", 0XFF0023FD},
				{"DREAMCASTNICK", 0XFF0023FD},
			};
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				if (pad_state.press & PAD_UP)
				{
					...
					if (menu.select > 0)
					{
						if (menu_options[menu.select - 1].text[0] == '\0') //if the previous option have nothing instead text, skip that
							menu.select -= 2;
						else
							menu.select -= 1;
					}
				...
				if (pad_state.press & PAD_DOWN)
				{
					...
					if (menu.select < COUNT_OF(menu_options) - 1)
					{
						if (menu_options[menu.select + 1].text[0] == '\0') //if the next option have nothing instead text, skip that
							menu.select += 2;
						else
							menu.select += 1;
					}
		...
		case MenuPage_Options:
		{
			...
			static const struct
			{
				...
				{OptType_Boolean, "SHOW TIMER", &stage.showtimer, {.spec_boolean = {0}}},
			};
